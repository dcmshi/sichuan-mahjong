...
### 7.2 Medium bot (Phase 10+, deferred polish)

- Use `ukeire(...)` for tile efficiency on discards.
...
- 27 tile-face SVGs from Wikimedia Commons (CC-BY-SA), bundled in `client/public/tiles/`. Plus `back.svg` for hidden tiles.
- File naming: `man-1.svg`, `pin-7.svg`, `sou-9.svg`, `back.svg`.
- `<Tile suit="pin" rank={5} faceUp={true} selected={false} />` component.
- Attribution page at `/about` driven by `credits.json`.

### 8.3 Interactions

- Tap a hand tile: select (visual lift). Tap again or tap discard zone: discard.
...
### 8.5 Animations (Framer Motion)

- Tile draw: slide from wall into hand.
- Discard: slide from hand to discard pool.
- Claim: discarded tile flies to claimer's meld zone.
- Hu: brief celebration (gentle, no sound by default — sounds are opt-in toggle).
...
SQLite via `better-sqlite3`, single file at user data dir:
...
- mDNS broadcast as `mahjong.local:8080` via `multicast-dns` package. Most home networks pick this up via Bonjour (Apple) / Avahi (Linux). Windows mDNS support is patchy; provide fallback IP URL.
...
- **Config:** CLI flags `--port`, `--no-mdns`, `--no-tailscale`, `--data-dir`. All optional with sensible defaults.
...
- **Replay tests:** canned action logs from real games → expected end states. Build a corpus during Phase 4. Include at least one game per fan combination from §5.8.
...
- One Playwright test: spin server, open 4 browser contexts (1 host + 3 bots, or 1 host + 3 joiners), play one full round to round-end screen.

### 11.4 Packaging

- Smoke test: `npx sichuan-mahjong --help` runs in CI.
- Binary smoke: each compiled binary launches and prints expected startup banner.

### 11.5 CI

GitHub Actions: lint → typecheck → test (vitest) → build (engine, server, client) → e2e (playwright) → package smoke.

---

## 12. Build phases

Each phase produces a working artifact. Engine work front-loaded since it's the riskiest part.

### Phase 0 — Scaffold
- pnpm workspace. Three packages with empty src + biome + vitest configured.
- `tiles.ts` complete: encoding, `buildWall(seed)`, sort helpers. 100% covered.
- `rng.ts` (xoshiro128**, seedable from string).
- README with dev commands.

### Phase 1 — Engine: basic round (no claims, no Hu)
- `state.ts`, `actions.ts` for: deal → declareVoid (atomic with first-discard) → play loop (East no-draw turn 1, all others draw → discard) with both strict and lenient void enforcement.
- No claims. No Hu detection. Round ends only on wall exhaustion.
- Test: a deterministic seeded game runs to wall exhaustion with no rule violations under both modes.

### Phase 2 — Engine: Hu detection
- `hand.ts`: `isWinningHand`, `findAllWinningArrangements`, `isTenpai` (with exhaustive-wait), `ukeire`.
- Property tests for hand detection.
- Self-draw Hu wired in.
- Single-winner round end (simplified scoring: 1 base point).

### Phase 3 — Engine: claims
- Pung, exposed kong, concealed/promoted/postponed kong, added-kong claims.
- Claim window resolution (priority Hu > Kong > Pung; counter-clockwise tiebreak).
- Hu via discard.
- Robbing-the-kong window (promoted AND postponed).
- Furiten / skip-Hu state tracking with `minFanToOverride`.
- Kong restrictions (no replacement → no kong; pung-then-kong-blocked).

### Phase 4 — Engine: bloody-to-end + scoring
- Multi-winner same-discard.
- Multi-Hu turn-passing per PDF page 22.
- Sit-out state; kong payouts to seated-out players; refund logic (three paths per §5.9).
- Full Novikov scoring (all 10 fan combinations from §5.8).
- Compatibility matrix enforcement (encoded from PDF Table 9).
- Heavenly / Earthly hand auto-cap (when `enableHeavenlyEarthly`).
- Void-suit-at-end penalty (lenient mode, with all-void-discards carve-out).
- Set-with-void penalty.
- False-Hu penalty (flat 8 per remaining non-Hu player).
- Wall-end edge cases (last-tile pung chain).
- Theoretical max hand value calc.
- Bu-ting payouts (TMV-based).
- Huan San Zhang phase.
- Build replay-test corpus covering each fan combo + each penalty path.

### Phase 5 — Server (local-host)
- Fastify HTTP + ws gateway.
- Lobby create/join, token issuance.
- `GameRoom` holds GameState; broadcasts `PlayerView` after each action.
- 60s reconnect window.

### Phase 6 — Client v0
- Vite + React + Tailwind scaffold.
- All screens (Landing → HostSetup → Join → Lobby → Game → RoundEnd).
- Tile rendering with SVGs, hand display, discard pool, claim panel.
- Zustand store wired to WebSocket.
- One full game playable in browser with 4 humans on LAN.

### Phase 7 — Bots
- Easy bot driver server-side (per §7.1).
- Host UI for adding/removing bots.
- Single-player practice mode (auto-fill 3 bots).

### Phase 8 — Persistence + replay
- SQLite write on roundEnd to user data dir.
- `/api/replay/:id` serves action log.

### Phase 9 — Networking & distribution
- LAN binding + IP detection + mDNS + QR code in CLI.
- Tailscale detection (`tailscale status --json` + interface fallback).
- TLS cert provisioning (`tailscale cert`).
- HTTPS listener on Tailscale path.
- Startup banner with all share URLs.
- npm package publication.
- Bun-compile binary build pipeline + GitHub Releases.

### Phase 10 — Polish
- PWA manifest + install prompt + offline shell (Tailscale path only, since PWA needs HTTPS).
- Sound effects (opt-in): tile click, kong, Hu.
- Reconnection toast UX.
- Score history across rounds (in-memory per match).
- Animation pass.
- "How to Play" help overlay sourced from §5.
...
4. **i18n** — English only in v1. Tile names use English + pinyin tooltips. Structure code so adding zh-CN later is mechanical.
...
8. **Heavenly/Earthly canonical-only mode** — v1 ships with cap-fan auto-win when enabled. If a user prefers strict-canonical scoring (where these score at structural fan), set `enableHeavenlyEarthly = false` and the engine treats them as ordinary first-turn Hu's.
