...
time. Per-item detail is in [TODO.md](../TODO.md), newest first.
...
28 SVGs, back down from 57 now the flat derivatives are gone. §13 says the CC-BY-SA
boundary depends on those staying "standalone fetched assets" and not being merged
"into compiled JavaScript output". Both cannot be true. **A licensing call, not a
