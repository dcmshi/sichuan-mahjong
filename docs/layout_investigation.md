...
**`prototype-shots/` is gitignored** (2026-08-04). A session comparing options
accumulates tens of megabytes of PNGs that are working evidence rather than a
deliverable; the findings live in this file and the probe regenerates the images
on demand. Contrast `docs/*.png`, which *are* committed — those are the README's
screenshots, generated by `pnpm shots`.
...
## 18. Two decisions, so they are not relitigated (2026-08-04)

### 18.1 N39 stays as it is — deliberately

Reviewed and **left**, with the reasoning rather than by neglect.

N40 changed what N39 even is. The tray's height now stops at six tiles however
deep the pile gets, because the second row of six grows *sideways* — so discard
depth no longer drives height at all, and the catastrophic case (a pile drawing as
a stack of black outlines) is gone. What can still squash a side column is
**melds**, and nothing else.

What is left is one device. The `max-height: 600px` shrink governs the 320×568
iPhone SE 1st gen and nothing else in the matrix; every other phone is ≥640 tall
and already draws 32px. There it costs 24px tiles where 32px would fit in the
common case, and a 3–4 meld seat overshoots by ~13px.

The three ways out were weighed:

| | Cost |
|---|---|
| Compute the count from the row height | A `ResizeObserver` and a layout effect inside components N38 memoised on purpose, plus tiles visibly vanishing from a pile mid-round as a meld lands |
| Make the art shrink with the box | The honest fix. Recorded as impossible because the art is rotated, so its on-screen height is its pre-rotation *width* — **worth re-examining**, since container query units (`cqh`) can express exactly that and could not when this was filed |
| Cap the meld row, then raise the tile size | Melds are only visible as those chips; making them scroll in an 80px column trades a rare squash for a permanent readability loss |

None is worth an observer in the hot path for one 2016 device. If a small-phone
player ever reports it, the container-query route is the one to try first, because
it fixes the root cause instead of budgeting around it. `sideSlack` and the
`box == art` check make any attempt verifiable in about 40 seconds.

### 18.2 A third row in the side rivers — tried, rejected on phones

A row is `--tile-w × 255/210` wide, so three at 32px need **117px** against the
80px a side column gets. Widening to 120px (160/176 on tablets) is the only way to
fit three at readable size, and that 40px per side comes out of the well:

```
375x667   ❌ 28 WALL CELLS UNDER DISCARD      wellFree 154 -> 231
390x844   ❌ 28
430x932   ❌ 14
768/810   ✅                                  (tablets have the width)
```

The well drops from ~183px to ~103px at 375. The wall frame is width-bound at
`min(96%, 22rem)` so it collapses with the well, and once it is that narrow its
mouth is smaller than the last discard — hence the cells underneath. `wellFree`
*rising* is the tell: the frame got shorter because it got narrower.

**And the third row is empty in every shot.** At the state the probe reaches, each
seat holds 7–9 discards, comfortably inside the existing twelve. A third row earns
something only for discards 13–18, the tail of a round, while ~40px of dead width
per side is paid on every frame of every round.

Tablets pass cleanly, so three rows *there* is defensible — but `RIVER_COLS` is a
