...
const kongHand = (): TileId[] => [
  tid(M(1), 1),
  tid(M(1), 2),
  tid(M(1), 3), // 3× man1 for exposed kong
  tid(P(1), 1),
  tid(P(2), 1),
  tid(P(3), 1),
  tid(P(4), 1),
...
describe('Phase 3 — pung claim', () => {
  ...
  it('CCW tiebreak: seat nearest CCW from discarder wins pung', () => {
    ...
    const hand1 = [
      ...
      ...Array.from(
        { length: 11 },
        (_, i) => tid(P(i + 1 <= 9 ? P(i + 1) : M(i - 8), 2)) as TileId,
      ),
    ];
    const hand3 = [
      tid(M(1), 0 as any),
      tid(M(1), 3),
      ...(Array.from({ length: 11 }, (_, i) => tid(P((i % 9) + 1), 3)) as TileId[]),
    ];
...
describe('Phase 3 — exposed kong claim', () => {
  it('exposed kong claim: forms kong meld and draws replacement', () => {
    ...
    expect(s.players[1]!.melds[0]!.kind === 'kong' && s.players[1]!.melds[0]!.subtype).toBe(
      'exposed',
    );
    // Replacement drawn
  ...
  it('kong claim has priority over pung claim', () => {
    ...
    const seat3h = [
      tid(M(1), 0 as any),
      tid(M(1), 3), // this conflicts with discard copy 0
      tid(P(1), 2),
      tid(P(2), 2),
      tid(P(3), 2),
      tid(P(4), 2),
...
describe('Phase 3 — concealed kong on own turn', () => {
  it('concealed kong: 4× same type in hand, forms kong, draws replacement', () => {
    ...
    expect(s.players[0]!.melds[0]!.kind === 'kong' && s.players[0]!.melds[0]!.subtype).toBe(
      'concealed',
    );
...
describe('Phase 3 — kong restrictions', () => {
  ...
  it('exposed kong claim rejected at wall end', () => {
    const s = makeState({ hands: [seat0Hand14(), kongHand(), [], []], wallEndReached: true });
...
describe('Phase 3 — wall-end edge cases', () => {
  ...
  it('wall-end claim window offers pung but not kong', () => {
    const seat1 = [
      tid(M(5), 1),
      tid(M(5), 2),
      tid(M(5), 3), // 3× man5: pung- AND kong-shaped
      tid(P(1), 0),
      tid(P(2), 0),
      tid(P(3), 0),
      tid(P(4), 0),
...
describe('Phase 3 — Heavenly Hand', () => {
  it('East declares heavenly hand on turn 1 with usedIndicator', () => {
    ...
    if (r.ok) {
      ...
      expect(r.state.players[0]!.hu!.handValue).toBe(2 ** 3); // 2^fanCap
    }
  });
...
describe('Phase 3 — Earthly Hand', () => {
  const earthlyHand = (): TileId[] => [
    ...
    tid(M(4), 2), // pung man4
    tid(P(1), 0),
    tid(P(1), 1), // pair pin1 (just drawn)
  ];
  ...
  it('non-dealer wins on first self-draw → earthly, auto cap-fan when enabled', () => {
    ...
    if (r.ok) {
      ...
      expect(r.state.players[1]!.hu!.handValue).toBe(2 ** 3); // 2^fanCap
    }
  });
...
describe('Phase 3 — full game with claims', () => {
  ...
  it('tile conservation holds throughout game', () => {
    ...
    const inMelds = final.players.reduce((sum, p) => {
      return (
        sum + p.melds.reduce((ms, m) => ms + (m.kind === 'kong' ? 4 : m.kind === 'pung' ? 3 : 3), 0)
      );
    }, 0);
    ...
    fc.assert(
      fc.property(
        fc
          ...
          .filter(t => Math.floor(t / 9) !== 2), // not sou
...
describe('Furiten bypass guard (A7)', () => {
  it('cannot declare a self-draw Hu right after a pung (no wall draw occurred)', () => {
    // Seat 1 is tenpai on a shanpon: punging the discarded M4 completes the hand
    // (M1/M2/M3 pungs + M4 pung + P5 pair). A player must NOT be able to pung and
    // then declare a "self-draw" Hu — that would launder a discard win into a
    // self-draw (bypassing furiten and pocketing the self-draw bonus).
    const p1Hand: TileId[] = [
      tid(M(1), 0),
      tid(M(1), 1),
      tid(M(1), 2),
      ...
      tid(M(3), 2),
      tid(M(4), 0),
      tid(M(4), 1), // pung pair, completed by the discard
      tid(P(5), 0),
      tid(P(5), 1), // pair
    ];
    const p0Hand: TileId[] = [tid(M(4), 2), tid(M(7), 0), tid(M(8), 0)];

    let state = makeState({ hands: [p0Hand, p1Hand, [], []], turn: 0 });

    // Seat 0 discards M4 → opens a claim window (seat 1 can pung or Hu).
    let r = applyAction(state, { t: 'discard', seat: 0, tile: tid(M(4), 2) });
    expect(r.ok).toBe(true);
    state = (r as { ok: true; state: GameState }).state;
    expect(state.pendingClaims).not.toBeNull();

    // Seat 1 chooses to PUNG (not Hu). Seats 2/3 have empty hands → auto-passed.
    r = applyAction(state, { t: 'claim', seat: 1, claim: { kind: 'pung' } });
    expect(r.ok).toBe(true);
    state = (r as { ok: true; state: GameState }).state;
    expect(state.turn).toBe(1);
    expect(state.turnDrawNeeded).toBe(false); // pung leaves the turn without a draw
    expect(state.drewThisTurn).toBe(false);

    // The hand IS structurally winning now — but declareHuOnDraw must be rejected
    // because seat 1 never drew this turn.
    const hu = applyAction(state, { t: 'declareHuOnDraw', seat: 1 });
    expect(hu.ok).toBe(false);
    if (!hu.ok) expect(hu.reason).toBe('must_draw_first');

    // A15: the punged tile moved into seat 1's meld and must not linger in
    // seat 0's discard pond (it would otherwise render in both places).
    expect(state.players[0]!.discards).not.toContain(tid(M(4), 2));
    expect(state.players[1]!.melds.some(m => m.kind === 'pung')).toBe(true);
  });
});

describe('Furiten override threshold (A16)', () => {
  it('a second, larger skipped Hu raises minFanToOverride to the max (keeps `since`)', () => {
    // Seat 1 is already furiten from a small (1-fan) skipped Hu. It now passes on a
    // bigger Hu — an all-man AllPungs + FullFlush win (3 fan) completed by a punged
    // M8. The override bar must rise to 3, else the player could later win on the
    // very 3-fan hand they just declined.
    const p1Hand: TileId[] = [
      tid(M(1), 0),
      tid(M(1), 1),
      tid(M(1), 2),
    ...
    const p0Hand: TileId[] = [tid(M(8), 2), tid(P(5), 0), tid(P(6), 0)];

    let state = makeState({ hands: [p0Hand, p1Hand, [], []], turn: 0 });
    state.players[1]!.furiten = { since: 5, minFanToOverride: 1 };

    // Seat 0 discards M8 → seat 1 could Hu (AllPungs + FullFlush) but passes.
    let r = applyAction(state, { t: 'discard', seat: 0, tile: tid(M(8), 2) });
    expect(r.ok).toBe(true);
    state = (r as { ok: true; state: GameState }).state;
    expect(state.pendingClaims).not.toBeNull();

    r = applyAction(state, { t: 'pass', seat: 1 });
    expect(r.ok).toBe(true);
    state = (r as { ok: true; state: GameState }).state;

    expect(state.players[1]!.furiten?.minFanToOverride).toBe(3); // raised from 1 → 3
